"""Browse RSS_2026 candidate grasps interactively (copy of view_bodex.py).

Hardcoded to the RSS_2026 candidate tree, which is laid out WITHOUT a hand level
(``candidates/{version}/{obj}/{scene_type}/{scene_id}/{grasp}``) and whose scene
ids don't line up with the AutoDex scene JSONs. So the hand is fixed to allegro,
the object is shown at its canonical table pose, and grasp + squeeze trajectory
are viewable (Show Trajectory).

Usage:
    python src/visualization/grasp_generation/view_grasp_rss.py --port 8080
"""
...
# RSS_2026 candidates: {root}/{version}/{obj}/{scene_type}/{scene_id}/{grasp}
# (no hand level; these are all allegro)
RSS_CANDIDATES_ROOT = "/home/mingi/RSS_2026/candidates"
FIXED_HAND = "allegro"

SOURCE_ROOTS = {
    "rss2026": RSS_CANDIDATES_ROOT,
}
...
class BODexBrowser(ViserViewer):
    ...
    def _source_root(self):
        return SOURCE_ROOTS.get(self.current_source, RSS_CANDIDATES_ROOT)

    def _data_root(self):
        """RSS_2026 has no hand level: root for current version = {source}/{version}"""
        if not all([self.current_source, self.current_version]):
            return ""
        return os.path.join(self._source_root(), self.current_version)
    ...
    def _on_source_change(self):
        ...
        # RSS_2026 has no hand level; hand is fixed to allegro.
        self.hand_selector.options = [FIXED_HAND]
        self.hand_selector.value = FIXED_HAND
        self._on_hand_change()

    def _on_hand_change(self):
        self.current_hand = FIXED_HAND
        # Versions live directly under the source root (no hand dir).
        versions = self._list_dirs(self._source_root())
    ...
    def _fallback_table_pose(self):
        """Canonical standing pose from scene/table/{4,0}.json, else identity."""
        for sid in ("4", "0"):
            p = os.path.join(get_scene_dir(self.current_hand, self.current_obj, "table"), f"{sid}.json")
            if os.path.exists(p):
                with open(p) as f:
                    cfg = json.load(f)
                return cart2se3(cfg["scene"]["mesh"]["target"]["pose"])
        return np.eye(4)
    ...
    def _load_scene(self):
        ...
        if os.path.exists(scene_json_path):
            with open(scene_json_path, "r") as f:
                cfg = json.load(f)
            scene = cfg["scene"]
            obj_pose = cart2se3(scene["mesh"]["target"]["pose"])
            cuboids = scene.get("cuboid", {})
        else:
            # RSS scene ids don't match AutoDex scene JSONs — fall back to a
            # canonical standing (table) pose so the grasp is still viewable.
            obj_pose = self._fallback_table_pose()
