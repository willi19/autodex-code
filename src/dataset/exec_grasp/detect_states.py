...
for d in sorted(glob.glob('/home/mingi/shared_data/autodex_dataset/selected_100/apple/*/')):
    m=json.load(open(f'{d}/executed_grasp/meta.json'))
    states,st=detect(d, m['cand_path'])
    t0=states[0][1]
    print(os.path.basename(d.rstrip('/')),':', [(s,f'{t-t0:+.2f}s') for s,t in states])
